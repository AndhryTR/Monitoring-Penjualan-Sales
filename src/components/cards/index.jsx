...
import { DrilldownButton, CollapsibleSection } from "../ui/index.jsx";
...
export function Leaderboard({ rows, colors, onDrilldown, onExportScorecard }) {
  ...
  return (
    <div className="sm-card p-5 sm-fadeup mb-8">
      <CollapsibleSection id="cards.leaderboard" title="Leaderboard Sales" sub="Diurutkan berdasarkan pencapaian (ACH%)" icon={Trophy} colors={colors}>
  ...
  return (
    <div className="sm-card p-5 sm-fadeup mb-6">
      <CollapsibleSection
        ...
        sub={active.note || "Ekstrapolasi linear dari rata-rata realisasi harian saat ini"}
        icon={Rocket}
        colors={colors}
        actions={
...
export function AlertsPanel({ alerts, colors, onDrilldown }) {
  ...
  return (
    <div className="sm-card p-5 sm-fadeup mb-6" style={{ borderColor: colors.coral + "44" }}>
      <CollapsibleSection id="cards.alerts" title="Perlu Perhatian" sub={`${alerts.length} item belum ada realisasi sama sekali`} icon={BellRing} colors={colors}>
        <div className="space-y-2">
          {visible.map((a, i) => (
...
export function PeriodComparisonCard({ comparison, colors, onClear }) {
  ...
  return (
    <div className="sm-card p-5 sm-fadeup mb-6">
      <CollapsibleSection
        id="cards.periodComparison"
        title="Bandingkan Periode"
        sub={`vs ${comparison.label}`}
        icon={History}
        colors={colors}
        actions={<button onClick={onClear} className="sm-btn p-2 rounded-full" style={{ background: colors.surface2 }}><X size={14} /></button>}
      >
