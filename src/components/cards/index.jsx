...
export function Leaderboard({ rows, colors, onDrilldown, onExportScorecard }) {
  ...
  const medal = (i) => ["🥇", "🥈", "🥉"][i] || `${i + 1}`;
  return (
    <div className="glass-panel p-5 sm-fadeup mb-8">
      <SectionTitle title="Leaderboard Sales" sub="Diurutkan berdasarkan pencapaian (ACH%)" icon={Trophy} colors={colors} />
      <div className="space-y-2">
        {ranked.map((sm, i) => (
          <div key={sm.code} className="flex items-center gap-3 px-3 py-2.5 rounded-xl sm-row" style={{ background: i < 3 ? colors.gold + "0D" : "transparent" }}>
...
export function ProjectionCard({ projection, colors, method = "linear", onMethodChange }) {
  ...
  return (
    <div className="glass-panel p-5 sm-fadeup mb-6">
      <div className="flex items-center justify-between flex-wrap gap-3 mb-1">
        ...
        <div className="flex p-1 rounded-xl shrink-0" style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.08)" }}>
          {PROJECTION_METHODS.map((m) => {
            ...
            return (
              <button key={m.key}
                ...
                style={{ background: isActive ? "rgba(255,255,255,0.10)" : "transparent", color: isActive ? colors.gold : colors.textMuted }}>
                {m.label}
              </button>
            );
          })}
...
export function AlertsPanel({ alerts, colors, onDrilldown }) {
  ...
  return (
    <div className="glass-panel p-5 sm-fadeup mb-6" style={{ borderColor: colors.coral + "44" }}>
      ...
      {alerts.length > 2 && (
        <button onClick={() => setExpanded(!expanded)} className="text-xs font-medium mt-3" style={{ color: colors.gold }}>
  ...
  return (
    <div className="glass-panel p-5 sm-fadeup mb-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={onClear} className="glass-btn p-2 rounded-full"><X size={14} /></button>
      </div>
      ...
      <div className="space-y-1.5 max-h-64 overflow-y-auto">
        {comparison.bySales.map((s) => (
          <div key={s.code} className="flex items-center justify-between text-sm px-3 py-2 rounded-lg" style={{ background: "rgba(255,255,255,0.06)", opacity: s.isGone ? 0.6 : 1 }}>
