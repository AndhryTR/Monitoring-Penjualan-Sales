...
export function Sidebar({ activeTab, onChangeTab, collapsed, onToggleCollapse, onOpenHistory, onOpenSettings, historyDisabled, colors }) {
  ...
  return (
    <aside
      className="glass-sidebar hidden md:flex flex-col shrink-0 sticky top-0 h-screen transition-all duration-300 ease-out z-10"
      style={{ width }}
    >
      ...
      <nav className="flex-1 overflow-y-auto px-2.5 pb-3">
        {SIDEBAR_SECTIONS.map((section) => (
          <div key={section.label} className="mb-4">
            {collapsed ? (
              <div className="mx-1.5 my-2 border-t" style={{ borderColor: "rgba(255,255,255,0.06)" }} />
            ) : (
              ...
              {section.items.map((item) => {
                ...
                return (
                  <button
                    ...
                    className="flex items-center gap-2.5 px-2.5 py-2 rounded-xl text-sm font-medium text-left disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-200"
                    style={{
                      background: active ? colors.gold + "18" : "transparent",
                      color: active ? colors.gold : colors.text,
                      justifyContent: collapsed ? "center" : "flex-start",
                      boxShadow: active ? `0 0 16px ${colors.gold}25` : "none",
      ...
      <button
        ...
        className="flex items-center gap-2.5 px-4 py-3 text-sm shrink-0"
        style={{ color: colors.textMuted, borderTop: "1px solid rgba(255,255,255,0.06)", justifyContent: collapsed ? "center" : "flex-start" }}
      >
